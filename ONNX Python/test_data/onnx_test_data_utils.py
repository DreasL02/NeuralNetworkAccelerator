# https://github.com/microsoft/onnxruntime/blob/a25f50fd464d8d63bc970d875d4caa2cf4b2e169/tools/python/onnx_test_data_utils.py
...
def get_arg_parser():
    ...
    parser.add_argument('--action', help='Action to perform',
                        choices=['dump_pb', 'numpy_to_pb', 'image_to_pb',
                                 'random_to_pb', 'update_name_in_pb'],
                        required=True)

    parser.add_argument('--input', help='The input filename or directory name')
    parser.add_argument(
        '--name', help='The value to set TensorProto.name to if creating/updating one.')
    parser.add_argument(
        '--output', help='Filename to serialize the TensorProto to.')
...
if __name__ == '__main__':
    ...
    if args.action == 'dump_pb':
        ...
        if not args.input or not args.output or not args.name:
            print(
                "Missing argument. Need input, output and name to be specified.", file=sys.stderr)
            sys.exit(-1)
        ...
        if not args.input or not args.output or not args.name:
            print(
                "Missing argument. Need input, output, name to be specified.", file=sys.stderr)
            sys.exit(-1)

        img_np = image_to_numpy(args.input, args.resize,
                                args.channels_last, args.add_batch_dim)
        ...
        if not args.output or not args.shape or not args.datatype or not args.name:
            print(
                "Missing argument. Need output, shape, datatype and name to be specified.", file=sys.stderr)
            sys.exit(-1)

        data = create_random_data(
            args.shape, args.datatype, args.min_value, args.max_value, args.seed)
        numpy_to_pb(args.name, data, args.output)
    elif args.action == 'update_name_in_pb':
        if not args.input or not args.name:
            print("Missing argument. Need input and name to be specified.",
                  file=sys.stderr)
            sys.exit(-1)
...
# Example usage:
# python onnx_test_data_utils.py --action dump_pb --input test_data/conv2d_input.pb
# python onnx_test_data_utils.py --action numpy_to_pb --input test_data/conv2d_input.npy --name input --output test_data/conv2d_input.pb
# python onnx_test_data_utils.py --action image_to_pb --input test_data/image.jpg --name input --output test_data/image.pb
# python onnx_test_data_utils.py --action random_to_pb --shape 1,3,224,224 --datatype f4 --name input --output test_data/random.pb
# python onnx_test_data_utils.py --action update_name_in_pb --input test_data/conv2d_input.pb --name new_name --output test_data/conv2d_input.pb
