...
class DirectMatMulSpec extends AnyFreeSpec with ChiselScalatestTester {
  // ======= configure the test =======
  val w = 8
  val wResult = 4 * w
  val numberOfRows = 2
  ...
  for (testNum <- 0 until numberOfTests) {
    ...
    "DirectMatMul should calculate correctly for test %d".format(testNum) in {
      test(new DirectMatMul(w = w, wResult = wResult, numberOfRows = numberOfRows, numberOfColumns = numberOfColumns, commonDimension = matrixCommonDimension, signed = signed)) { dut =>
