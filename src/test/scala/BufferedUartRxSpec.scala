...
class BufferedUartRxSpec extends AnyFreeSpec with ChiselScalatestTester {
  ...
  "Should support a single byte buffer" in {
    test(new BufferedUartRxForTestingOnly(frequency, baudRate, bufferByteSize = 1)) { dut =>
      ...
      bitsToSend.foreach { bit =>
        ...
        var i = 0
        while (i < cyclesPerSerialBit & !dut.io.outputChannel.valid.peek().litToBoolean) {
          i = i + 1
          dut.clock.step(1)
        }
      }
  ...
  "Should support a two byte buffer" in {
    test(new BufferedUartRxForTestingOnly(frequency, baudRate, 2)) { dut =>
      ...
      val bitsToSend = scala_utils.UartCoding.encodeBytesToUartBits(Array(testValue1, testValue2))
      println("Sending bit vector: " + bitsToSend)
      ...
      dut.io.outputChannel.bits(0).expect(testValue1.U)
      dut.io.outputChannel.bits(1).expect(testValue2.U)
    }
  }
  ...
  "Should support a nine byte buffer" in {
    test(new BufferedUartRxForTestingOnly(frequency, baudRate, 9)) { dut =>
      ...
      val bitsToSend = scala_utils.UartCoding.encodeBytesToUartBits(newMemoryBytesToSend)
      println("Sending bit vector: " + bitsToSend)
      ...
      for (i <- 0 until 9) {
        dut.io.outputChannel.bits(i).expect(newMemoryBytesToSend(i).U)
      }
    }
  }
  ...
  "Should support multiple buffer fills of one byte" in {
    test(new BufferedUartRxForTestingOnly(frequency, baudRate, 1)) { dut =>
      ...
      val bitsToSend = scala_utils.UartCoding.encodeBytesToUartBits(Array(testValue1))
      println("Sending bit vector: " + bitsToSend)
      ...
      val bitsToSend2 = scala_utils.UartCoding.encodeBytesToUartBits(Array(testValue2))
      ...
      dut.io.outputChannel.bits(0).expect(testValue2.U)
    }
  }
  ...
  "Should support multiple buffer fills of two bytes" in {
    test(new BufferedUartRxForTestingOnly(frequency, baudRate, 2)) { dut =>
      ...
      val bitsToSend2 = scala_utils.UartCoding.encodeBytesToUartBits(Array(testValue3, testValue4))
      println("Sending bit vector: " + bitsToSend2)
      ...
      dut.io.outputChannel.bits(0).expect(testValue3.U)
      dut.io.outputChannel.bits(1).expect(testValue4.U)
    }
  }
}
