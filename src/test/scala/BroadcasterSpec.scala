...
class BroadcasterSpec extends AnyFreeSpec with ChiselScalatestTester {
  ...
  "Broadcaster should broadcast correctly (scalar)" in {
  ...
  "Broadcaster should broadcast correctly (vector)" in {
        ...
        for (j <- 0 until test2(1)._2) {
          for (k <- 0 until test2(1)._3) {
            for (l <- 0 until test2(1)._4) {
              dut.io.resultChannel.bits(i)(j)(k)(l).expect(test2Data(i % test2(0)._1)(j % test2(0)._2)(k % test2(0)._3)(l % test2(0)._4).U)
            }
          }
        }
      }
