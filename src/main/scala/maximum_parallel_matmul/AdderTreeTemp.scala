...
class AdderTreeTemp(
  ...
  private val resultReg = RegInit(0.U(w.W))
  ...
  when(io.resultChannel.ready) {
    when(io.inputChannel.valid) {
      busy := true.B
    }
  }.elsewhen(io.resultChannel.valid && io.inputChannel.ready) {
    busy := true.B
    io.resultChannel.valid := true.B
  }.elsewhen(!io.resultChannel.valid) {
    when(done) {
      busy := false.B
    }
  }
